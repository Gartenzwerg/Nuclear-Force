...
int main(){
	long N = 200001;
	long double h = 0.001;		//step size: 0.01fm
	...
	*/
	...
	ofstream fout;
	fout.open("res_300_0.001.dat");		
	for(int i = 0; i < 300; i++) {		//scan the energy interval between 0.1 MeV and 4.0 MeV
		SecondOrderODESolver(u_0, u_1, h, N, l, E);
		fout<<E<<"	"<<PhaseShift(u_0[200000]/u_1[200000], E, l, 200)<<endl;
		E += 0.5;
	}
	fout.close();
}
...
long double V(long double r) {
	long double res = 0;
	res += -10.463*exp(-0.7*r)/(0.7*r);
	res += -1650.6*exp(-4*0.7*r)/(0.7*r);
	res += 6484.3*exp(-7*0.7*r)/(0.7*r);
	return res;
}
...
long double f(int l, long double r, long double E){
	...
	return l*(l+1.0)/(r*r)+0.0241218949*(-E+V(r));
}
...
long double PhaseShift(long double R, double E, int l, double matching_radius){
	long double wave_number = sqrt(0.0241218949*E);
